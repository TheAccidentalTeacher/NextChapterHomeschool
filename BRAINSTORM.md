...
| R-NEW1 ✅ LOCKED | **How does rotation handle uneven absences?** | ✅ Calendar holds. Miss an epoch, miss that role. Rotation doesn't shift. Portfolio tracks gaps. See Decision 77. |
...
| # | Question | Why It Matters |
|---|---------|---------------|
| A1 ✅ LOCKED | **Absent role → present teammate absorbs it (team votes; Scott assigns if no consensus). No AI fill. No vacancy.** | See Decision 71. |
| A2 ✅ LOCKED | **No ghost civs. Every absent role is redistributed to a present teammate. Someone is always covering.** | See Decision 71. |
...
| 2 | **Class structure** | Two separate classes on one shared map | 6th grade (~15 students) covers the Americas. 7/8 grade (~22 students) covers Old World — Asia, Europe, Africa. Same map. Different playing times. Contact between classes is teacher-controlled. |
...
| 8 | **Team persistence** | Yes — saved across sessions | Game runs ~3 weeks post spring break. State must persist between class periods. |
...
| 12 | **The big colonial moment** | Teacher controls it deliberately | When Scott is ready to teach colonization, he fires the contact event between 6th grade (Americas) and 7/8 grade (Old World). The contact event is teacher-controlled for exactly this reason — the curriculum gets there when the teacher says it does. |
...
| 18 | **Role rotation** | Roles rotate every epoch | Every student plays every role across the 3-week project. Historically rich — students experience what it costs to be the General vs. the Merchant vs. the Scholar. Absence complication: if a student misses an epoch, do they skip that role in the rotation or does the schedule hold and they just miss it? **See R-NEW1.** |
...
| 53 | **Contact awareness: both classes represented on every continent, fair game from turn one** | No isolated Americas. No "Old World arrives, New World reacts." Both 6th grade and 7/8 have civilizations spread across all continents from the start. Every student knows the other class is on the same map and that contact is possible at any time. Contact becomes a collision between peers who saw each other coming — not a discovery event where one side has all the power and the other has none. Both sides are peers on a globe-spanning map with full awareness and full agency from day one. |
...
| 66 | **Cross-class timing: Option C simultaneous resolution. Both classes submit → Pending. Scott RESOLVES every morning. Grace mode if missed. RESOLVE READY notification on DM panel.** | 6th grade submits their epoch during class → moves go to `resolution_status: 'pending'` — visible on Scott's DM panel, invisible to 7/8 as territory changes. 7/8 logs in and sees only the last fully RESOLVED epoch state. 7/8 submits their epoch → also Pending. Scott hits RESOLVE every morning during prep time. Both classes' pending submissions process simultaneously — same yield math, same d20 triggers, same NPC auto-pulses, cross-class conflicts (if contact is open) all resolve in one pass. Map updates. Haiku generates recap paragraphs for all teams. Exit hooks generated. Both classes log in that day to the resolved state and begin their next epoch. **What 7/8 sees during their class while 6th's epoch is pending:** the last fully resolved map only. 6th's pending territorial moves and unit deployments are invisible to 7/8. Exception: Wonder completions already announced on the 6th period projector are ambient public knowledge but the map doesn't reflect them until RESOLVE. **Morning RESOLVE sequence:** (1) Scott taps RESOLVE on DM panel. (2) Both pending epoch submissions process simultaneously. (3) Cross-class conflicts resolve in this same pass. (4) Map updates: territory, units, resources, NPC positions. (5) Haiku recaps generated and stored. (6) All students see the resolved map on login. **Grace mode:** if Scott can't RESOLVE a given morning (sick day, field trip, emergency), submissions stay Pending — no data lost, nothing breaks. Two days' submissions queue. Next morning RESOLVE processes Day N first, then Day N+1 in sequence. Students see two epochs worth of history resolve at once — dramatic, not broken. **RESOLVE READY indicator:** when both classes have submitted, a notification appears on Scott's DM panel. He never has to check manually whether the queue is ready. DB: `epoch_submissions` gains `resolution_status: 'pending' | 'resolved'`, `resolved_at: timestamp`. `game_state` gains `pending_resolution_count: number`. |
...
- Project runs ~3 weeks post spring break
